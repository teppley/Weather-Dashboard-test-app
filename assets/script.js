...
function searchButtonClicked() {
  ...
  getWeather(city);
  //clears value once the search is activated
  searchInput.val("");
}
...
function getWeather(city) {
  ...
  if (city.country == null) {
    queryURLCurrent = "https://api.openweathermap.org/data/2.5/weather?q=" + city.city + "&units=imperial&appid=" + APIKey;
    queryURLForecast = "https://api.openweathermap.org/data/2.5/forecast?q=" + city.city + "&units=imperial&appid=" + APIKey;
  } else {
    queryURLCurrent = "https://api.openweathermap.org/data/2.5/weather?q=" + city.city + "," + city.country + "&units=imperial&appid=" + APIKey;
    queryURLForecast = "https:////api.openweathermap.org/data/2.5/forecast?q=" + city.city + "," + city.country + "&units=imperial&appid=" + APIKey;
  }
...
function buildCurrentWeather(data) {
  if (data != null) {
    ...
    currentWeatherDiv.append(
      ...
      , $("<p>").text("Wind Speed: " + (Math.round((units === impUnits) ? (data.wind.speed * 3.6) : data.wind.speed)) + " " + units.speed)
